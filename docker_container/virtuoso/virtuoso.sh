...
echo "Deploying Virtuoso for KE project"
...
do
	...
	then
    	if [ ! -f "${D}/.data_loaded" ] ;
    	then
    		echo "Loading DB data - segment ${D}" ;
    		echo "File in ${D}: ";
    		ls ${D}
    		echo "ld_dir_all('${D}', '*.gz', 'https://w3id.org/arco/data');" > /load_data.sql
			echo "rdf_loader_run();" >> /load_data.sql
			echo "exec('checkpoint');" >> /load_data.sql
			echo "WAIT_FOR_CHILDREN; " >> /load_data.sql
			echo "$(cat /load_data.sql)"
...
if [ -d "/usr/local/virtuoso-opensource/share/virtuoso/vad/ontologies" ]; 
	...
	echo "ld_dir_all('/usr/local/virtuoso-opensource/share/virtuoso/vad/ontologies', '*.rdf', 'https://raw.githubusercontent.com/Lostefra/TranslationCoherence/main/ontology/');" > /load_data.sql
	echo "rdf_loader_run();" >> /load_data.sql
	echo "exec('checkpoint');" >> /load_data.sql
	echo "WAIT_FOR_CHILDREN; " >> /load_data.sql
	echo "$(cat /load_data.sql)"
...
if [ -d "/usr/local/virtuoso-opensource/share/virtuoso/vad/linking" ]; 
	then
    if [ ! -f "/usr/local/virtuoso-opensource/share/virtuoso/vad/linking/.data_loaded" ] ;
    then
    	echo "Loading DB data - segment linking" ;
    	echo "File in /usr/local/virtuoso-opensource/share/virtuoso/vad/linking: ";
   		...
   		echo "ld_dir_all('/usr/local/virtuoso-opensource/share/virtuoso/vad/linking', '*.gz', 'https://w3id.org/arco/data/linking');" > /load_data.sql
		echo "rdf_loader_run();" >> /load_data.sql
		echo "exec('checkpoint');" >> /load_data.sql
		echo "WAIT_FOR_CHILDREN; " >> /load_data.sql
		echo "$(cat /load_data.sql)"
		virtuoso-t +wait && isql-v -U dba -P "$pwd" < /load_data.sql
		kill $(ps aux | grep '[v]irtuoso-t' | awk '{print $2}')
		echo "`date +%Y-%m-%dT%H:%M:%S%:z`" > /usr/local/virtuoso-opensource/share/virtuoso/vad/linking/.data_loaded
	fi
fi
...
if [ -d "/usr/local/virtuoso-opensource/share/virtuoso/vad/quarantine" ]; 
	then
    if [ ! -f "/usr/local/virtuoso-opensource/share/virtuoso/vad/quarantine/.data_loaded" ] ;
    then
    	echo "Loading DB data - segment quarantine" ;
    	echo "File in /usr/local/virtuoso-opensource/share/virtuoso/vad/quarantine: ";
   		...
   		echo "ld_dir_all('/usr/local/virtuoso-opensource/share/virtuoso/vad/quarantine', '*.gz', 'https://w3id.org/arco/data/quarantine');" > /load_data.sql
		echo "rdf_loader_run();" >> /load_data.sql
		echo "exec('checkpoint');" >> /load_data.sql
		echo "WAIT_FOR_CHILDREN; " >> /load_data.sql
		echo "$(cat /load_data.sql)"
		virtuoso-t +wait && isql-v -U dba -P "$pwd" < /load_data.sql
		kill $(ps aux | grep '[v]irtuoso-t' | awk '{print $2}')
		echo "`date +%Y-%m-%dT%H:%M:%S%:z`" > /usr/local/virtuoso-opensource/share/virtuoso/vad/quarantine/.data_loaded
	fi
fi
...
#     echo "ld_dir_all('/usr/local/virtuoso-opensource/share/virtuoso/vad/ontologies/', '*.ttl', 'https://raw.githubusercontent.com/Lostefra/TranslationCoherence/main/ontology/');" >> /load_ontologies.sql
