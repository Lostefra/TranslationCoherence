...
def add_intensional_reification(class_node, result_graph):
    ...
    intensional_reification_class_node = term.URIRef(transl_coher_prefix + intensional_reification_class_name)
...
def add_intensional_reifications_parents(graphs, result_graph):
    for graph in graphs:
        for s, o in graph.subject_objects(constants.SUBCLASS_PREDICATE):
            if s in result_graph.all_nodes() and o in result_graph.all_nodes():
